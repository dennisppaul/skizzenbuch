...
static final boolean USE_P3D = true;
...
void draw() {
    ...
    if (USE_P3D) {
        ...
        project_on_screen(p3d.modelview, p3d.projection, mInput, width, height, mResult);
    ...
    project_on_screen(mModelview, mProjection, mInput, width, height, mResult);
...
void setup_matrices(PMatrix3D mModelview, PMatrix3D mProjection, float pWidth, float pHeight) {
...
void setup_modelview(PMatrix3D mModelview,
...
void setup_projection(PMatrix3D mProjection,
...
void setup_frustum(PMatrix3D mProjection,
    ...
    mProjection.set(n2 / w, 0, (right + left) / w, 0,
        0, -n2 / h, (top + bottom) / h, 0,
        0, 0, -(zfar + znear) / d, -(n2 * zfar) / d,
        0, 0, -1, 0);
}

void project_on_screen(PMatrix3D modelview, PMatrix3D projection, PVector pInput,
    float pWidth, float pHeight, PVector pResult) {
    ...
    project_on_screen(projection, ax, ay, az, aw, pWidth, pHeight, pResult);
}

void project_on_screen(PMatrix3D projection, float x, float y, float z, float w,
    float pWidth, float pHeight, PVector pResult) {
