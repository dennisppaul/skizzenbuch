...
ArrayList<Metaball> mBalls;

float mRotation = 0;

ArrayList<PVector> mLines;
...
void setup() {
  ...
  mLines = new ArrayList<PVector>();

  /* spawn metaballs */
  mBalls = new ArrayList<Metaball>();
  for (int i = 0; i < 10; i++) {
...
void draw() {
  ...
  mBalls.get(0).position.x = mouseX;
  mBalls.get(0).position.y = mouseY;
...
void drawLines(ArrayList<PVector> myLines, float pZ) {
  stroke(0, 128);
  pushMatrix();
  scale(mResolutionScaleX, mResolutionScaleY);
  strokeWeight(20.0/mResolutionScaleX);
  beginShape(LINES);
