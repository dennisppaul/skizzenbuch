...
static class MarchingSquares {
  ...
  static void lines(ArrayList<PVector> mLines, float[][] pGridValues, float pThreshold) {
      ...
      for (int y = 0; y < pGridValues[x].length - 1; y++) {
        ...
        if (square_idx != 0 || square_idx != 15) {
          ...
          while (mLine[square_idx][n] != -1) {
            ...
            p1.x /= mArrayDimensions.x; /* normalize positions */
            p1.y /= mArrayDimensions.y; /* normalize positions */
            p2.x /= mArrayDimensions.x; /* normalize positions */
            p2.y /= mArrayDimensions.y; /* normalize positions */
            mLines.add(p1);
            mLines.add(p2);
          }
        }
